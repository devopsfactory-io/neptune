...
- **once: false** (default): Neptune executes the step's `run` command in each changed stack directory.
- **once: true**: Run the command once (e.g. at repo root); use this if you invoke the Terramate CLI yourself, e.g. `run: terramate run --changed -- some-command`.

See [Neptune configuration – Step options: once](https://github.com/devopsfactory-io/neptune/blob/main/docs/configuration.md) for details.
