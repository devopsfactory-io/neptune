...
Contributing workflow (issues, pull requests, checklist) is described in [CONTRIBUTING.md](https://github.com/devopsfactory-io/neptune/blob/main/CONTRIBUTING.md). For pull requests, ensure your commits are signed off (DCO); see the [Sign-off (DCO)](https://github.com/devopsfactory-io/neptune/blob/main/CONTRIBUTING.md#pull-requests) section for git config and `git commit -s`.

## End-to-end tests

E2E tests run Neptune against MinIO via Docker Compose and do not require a real GitHub PR. See [e2e/README.md](https://github.com/devopsfactory-io/neptune/blob/main/e2e/README.md) for prerequisites and how to run `./e2e/scripts/run-terramate.sh`. PR emulation is done with an isolated git repo (main + pr-1 branch with changed stacks) and `NEPTUNE_E2E=1` so GitHub checks are skipped.
...
Use the Go version from `go.mod`. See [AGENTS.md](https://github.com/devopsfactory-io/neptune/blob/main/AGENTS.md) for code style, testing, and CI.
...
3. **GoReleaser runs automatically**: The [`.github/workflows/release.yml`](https://github.com/devopsfactory-io/neptune/blob/main/.github/workflows/release.yml) workflow triggers on tag push and runs GoReleaser with the configuration from [`.goreleaser.yml`](https://github.com/devopsfactory-io/neptune/blob/main/.goreleaser.yml).
...
4. **Release artifacts**: GoReleaser creates a [GitHub Release](https://github.com/devopsfactory-io/neptune/releases) with:
   ...
   - Release notes generated by **GitHub** (GoReleaser uses `changelog.use: github-native`), categorized by [`.github/release.yml`](https://github.com/devopsfactory-io/neptune/blob/main/.github/release.yml) and **PR labels** (Breaking Changes, Features, Bug fixes, Documentation, Dependency updates, Other work), and a Full Changelog link in the release footer
...
Release notes are generated by **GitHub** when GoReleaser runs (because of `changelog.use: github-native` in [.goreleaser.yml](https://github.com/devopsfactory-io/neptune/blob/main/.goreleaser.yml)). Categories come from [`.github/release.yml`](https://github.com/devopsfactory-io/neptune/blob/main/.github/release.yml) and the **labels on merged PRs** in the release range. To create the repository labels once, run from the repo root (requires [gh](https://cli.github.com/) and authentication):

```bash
./scripts/create-release-labels.sh
```

The [labeler](https://github.com/devopsfactory-io/neptune/blob/main/.github/workflows/labeler.yml) applies labels from [.github/labeler.yml](https://github.com/devopsfactory-io/neptune/blob/main/.github/labeler.yml): path-based (`dependencies`, `documentation`) and head-branch rules (`breaking-change` when branch contains `!`, `feature` for `feat*`, `enhancement` for `enhance*`, `bug` for `fix*` without `dep`, `github-actions` for `ci*`, `dependencies` for branch containing `(deps)`). To label already-merged (or other existing) PRs, run the **Label old PRs** workflow: Actions → "Label old PRs" → Run workflow. You can choose PR state (merged, closed, all) and a limit. The workflow first applies the same rules as the labeler to both head branch and PR title (since the labeler has no branch context on manual runs), then runs the labeler for path-based labels (see [actions/labeler](https://github.com/actions/labeler)). Contributors can use these branch names so PRs are auto-labeled; see [CONTRIBUTING.md](https://github.com/devopsfactory-io/neptune/blob/main/CONTRIBUTING.md#branch-naming-and-pr-labels).

### Testing the changelog

With `changelog.use: github-native`, the release body is produced by GitHub's API, which uses [`.github/release.yml`](https://github.com/devopsfactory-io/neptune/blob/main/.github/release.yml) for categories. The `groups`, `sort`, and `filters` in `.goreleaser.yml` are not used. To preview release notes without publishing:
