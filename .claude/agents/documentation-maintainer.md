...
3. **Apply updates** – Make concrete edits (README, docs, examples, AGENTS.md, CLAUDE.md, commands, skills) so they reflect the new behavior or structure.
...
7. **CLAUDE.md** – Update if coding conventions, mandatory rules (DCO, Go standards, CI/release, config schema), or project-wide guidance changed.
...
| CLAUDE.md | AI agents | Project conventions and mandatory rules (DCO, Go standards, CI/release, config schema) |
...
- **New env vars or config keys** → README, docs/ (configuration.md, object-storage.md), .neptune.example.yaml, examples/ if an example should show the new config, and CLAUDE.md (config schema section).
- **New Make targets or CI jobs** → AGENTS.md, CLAUDE.md (CI/release section), testing-and-ci skill.
