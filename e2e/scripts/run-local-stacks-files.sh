#!/usr/bin/env bash
# E2E for stacks_management: local (stack.hcl discovery). Same flow as run-terramate.sh but uses .neptune.localstacksfiles.yaml.
set -euo pipefail
...
cp "$E2E_DIR"/.neptune.localstacksfiles.yaml "$E2E_TMP/.neptune.yaml"
...
echo "Running neptune command plan (stacks_management: local)..."
neptune command plan
echo "Running neptune command apply (stacks_management: local)..."
neptune command apply
echo "E2E (local stacks files) completed successfully."
