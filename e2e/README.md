...
- **What they do**: Checkout (with full history so the base ref exists for Terramate), start MinIO and create the bucket, then run `./e2e/run-integration.sh`, which builds Neptune and runs `neptune command plan` and `neptune command apply` from the repo root with `NEPTUNE_CONFIG_PATH=e2e/.neptune.yaml`. GitHub env (`GITHUB_REPOSITORY`, `GITHUB_PULL_REQUEST_NUMBER`, etc.) is set from the workflow so Neptune posts result comments on the PR.
- **Full plan/apply**: If the PR changes files under `e2e/`, Terramate reports changed stacks and plan/apply run on them. If the PR does not change `e2e/`, Neptune may report no changed stacks, post a comment, and exit 0; the job still validates config load, GitHub client, and comment posting.
